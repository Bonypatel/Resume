...
\begin{cventries}
  ...
  \cventry
    ...
    {Different Defense Contractor}
    {City, State}
    {Jun. 2021 - Sep. 2022}
    {
      \begin{cvitems}
        \item {Worked on the Interoperability team to implement new features and uphold existing code, contributing in the development of the Proprietary Simulator used by the US Navy}
        ...
        \item {Incorporated new functionality to the Aegis Defense System to allow for testing with the Proprietary Simulator and performed the full scale lab testing}
  ...
  \cventry
    ...
    {
      \begin{cvitems}
        \item {Worked with the Command and Decision team to implement new features and maintain existing C++ code to modernize the Aegis Defense System used by the US Navy}
        \item {Rigorously tested and analyzed the structure of a dozen different components by performing tests ranging from unit tests to large scale production tests at a full-scale facility}
  ...
  \cventry
    ...
    {
      \begin{cvitems}
        \item {Increased software fidelity by developing a Python testing framework to automatically run stress tests of new software releases, which lead to increased productivity by removing the need for engineers to run tests manually}
        \item {Developed the framework in an object oriented manner so that it can be expanded easily by other engineers}
        \item {Created an automated reporting system deployed using Jenkins, which operated continuously for up to 96 hours, to meticulously compile and email reports encompassing all software versions tested}
      \end{cvitems}
    }
\end{cventries}
